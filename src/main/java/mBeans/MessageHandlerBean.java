...
public class MessageHandlerBean {
	...
	public LinkedList<UserMessage> getMessageList() {
		if(sessionData.isLogged()){
			...
			}else if(mailBoxB.getActiveDirectory().equals(MailBoxBean.SENT)){
				db.getSentMessageList(sessionData.getEmail(),messageList);
			}else{
				db.getMessageList(sessionData.getEmail(),messageList,MailBoxBean.DELETED);
			}
		}
		...
		for(Long id: toDelete.keySet()){
			try{
				...
				if(toDelete.get(id) !=null && toDelete.get(id)){
					...
					if(mailBoxB.getActiveDirectory().equals(MailBoxBean.RECEIVED)){
						db.copyToDeleted(message_id,sessionData.getEmail()
								,mailBoxB.getActiveDirectory());
						...
						db.deleteMessage(message_id,sessionData.getEmail()
								,mailBoxB.getActiveDirectory());
					}
				}
			}catch(Exception e){
	...
	public String deleteMessage(String id){	
		try{
			...
			if(mailBoxB.getActiveDirectory().equals(MailBoxBean.RECEIVED)){
				...
				db.deleteMessage(message_id, sessionData.getEmail(), MailBoxBean.DELETED);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String useDirectory(String directory){
		mailBoxB.setActiveDirectory(directory);
		System.out.println("Directory set"+"sent");
		return "messages?faces-redirect=false"; 
	}
